...
interface MatchContextType {
  ...
  // Custom Lineups & Upcoming Matches (Remote and Admin uploaded)
  customLineups: Record<'home' | 'away', Lineup>;
  setCustomLineups: (lineups: Record<'home' | 'away', Lineup>) => void;
  customUpcomingMatches: UpcomingMatch[];
  setCustomUpcomingMatches: (matches: UpcomingMatch[]) => void;
...
export const MatchProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setCustomLineups = (nextLineups: Record<'home' | 'away', Lineup>) => {
    ...
    setCustomLineupsState(nextLineups);
  };

  const setCustomUpcomingMatches = (nextMatches: UpcomingMatch[]) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setCustomUpcomingMatchesState(nextMatches);
  };
  ...
  const assetsVersionRef = useRef<number>(-1);
  ...
  useEffect(() => {
    ...
    // Guard: Prevent feedback loops by only pushing changes if they were initiated by local user actions recently
    ...
    const timer = setTimeout(() => {
      ...
      if (Date.now() - lastLocalActionTime.current > 2000) {
        return;
      }
  ...
  // Socket.io Real-time Instant Synchronizer
  useEffect(() => {
    ...
    socket.on('state:update', (data: any) => {
      ...
      const shouldApply = !isTyping && (isOverlayMode || (
        Date.now() - lastLocalActionTime.current > 2000
      ));
      if (!shouldApply) return;
      if (data.timestamp && data.timestamp === lastAppliedServerTimestamp.current) return;
      lastAppliedServerTimestamp.current = data.timestamp;

      localTimestampRef.current = data.timestamp;

      if (data.selectedApiMatchId !== undefined) setSelectedApiMatchId(data.selectedApiMatchId);
      if (data.streamSource !== undefined) setStreamSource(data.streamSource);
      if (data.selectedLineupTeam !== undefined) setSelectedLineupTeam(data.selectedLineupTeam);
      if (data.selectedDate !== undefined) setSelectedDate(data.selectedDate);
      if (data.isTokenSavingMode !== undefined) setIsTokenSavingMode(data.isTokenSavingMode);
    ...
    socket.on('score:update', (matchState: any) => {
    ...
    socket.on('upcoming:update', (upcomingMatches: any) => {
      if (!upcomingMatches) return;
      setCustomUpcomingMatchesState(upcomingMatches);
    });
  ...
  // 2. Bidirectional Mode: Pull updates from server periodically to synchronize multiple devices (like phone and PC)
  useEffect(() => {
    let isMounted = true;
    const fetchSharedState = async () => {
      try {
        ...
        // Overlay always updates. Controller only updates if we aren't actively editing
        const activeEl2 = document.activeElement;
        const isTyping2 = activeEl2 && (activeEl2.tagName === 'INPUT' || activeEl2.tagName === 'TEXTAREA');
        const shouldApply = !isTyping2 && (isOverlayMode || (
          Date.now() - lastLocalActionTime.current > 2000
        ));
        ...
        // Update local timestamp so we don't re-apply our own pulled update in a feedback loop
        if (!isOverlayMode) {
          localTimestampRef.current = data.timestamp;
        }

        if (data.selectedApiMatchId !== undefined) {
          setSelectedApiMatchId(prev => prev === data.selectedApiMatchId ? prev : data.selectedApiMatchId);
        }
        if (data.streamSource !== undefined) {
          setStreamSource(prev => prev === data.streamSource ? prev : data.streamSource);
        }
        if (data.selectedLineupTeam !== undefined) {
          setSelectedLineupTeam(prev => prev === data.selectedLineupTeam ? prev : data.selectedLineupTeam);
        }
        if (data.selectedDate !== undefined) {
          setSelectedDate(prev => prev === data.selectedDate ? prev : data.selectedDate);
        }
        if (data.isTokenSavingMode !== undefined) {
          setIsTokenSavingMode(prev => prev === data.isTokenSavingMode ? prev : data.isTokenSavingMode);
        }
        if (data.isPlayingSim !== undefined) {
          setIsPlayingSim(prev => prev === data.isPlayingSim ? prev : data.isPlayingSim);
        }
        if (data.phoneCamStreamId !== undefined) {
          setPhoneCamStreamId(prev => prev === data.phoneCamStreamId ? prev : data.phoneCamStreamId);
        }
        ...
        if (data.customUpcomingMatches) {
          setCustomUpcomingMatchesState(prev => JSON.stringify(prev) === JSON.stringify(data.customUpcomingMatches) ? prev : data.customUpcomingMatches);
        }
        if (data.matchState) {
          ...
          if (assetsRes.ok) {
            ...
            if (isMounted && assetsData) {
              if (assetsData.backgroundImage !== undefined) {
                setBackgroundImageState(prev => prev === assetsData.backgroundImage ? prev : assetsData.backgroundImage);
              }
              if (assetsData.competitionImage !== undefined) {
                setCompetitionImageState(prev => prev === assetsData.competitionImage ? prev : assetsData.competitionImage);
              }
    ...
    fetchSharedState();
    const interval = setInterval(fetchSharedState, 1500);
  ...
  useEffect(() => {
    ...
    if (apiMatches.length > 0) {
      ...
      if (!selectedApiMatchId || !alreadySelectedExists) {
  ...
  useEffect(() => {
    // Keep the timer active regardless of API stats links
    ...
    simTimerRef.current = setInterval(() => {
      setMatchState((prev) => {
        ...
        // 1st Half stop check: at 45:00 under 1H, trigger halftime and pause simulation
        if (currentShortStatus === '1H' && nMin >= 45) {
          setTimeout(() => setIsPlayingSim(false), 0);
          return {
            ...prev,
            status: 'HALF_TIME',
            shortStatus: 'HT',
        ...
        // 2nd Half stop check: at 90:00 under 2H, trigger finished and pause simulation
        if (currentShortStatus === '2H' && nMin >= 90) {
          setTimeout(() => setIsPlayingSim(false), 0);
          return {
            ...prev,
            status: 'FINISHED',
        ...
        return {
          ...
          minute: nMin,
          seconds: nSecs,
          extraTime1H: prev.extraTime1H,
          extraTime2H: prev.extraTime2H,
          stats: updatedStats,
  ...
  const togglePlay = async () => {
    ...
    if (isPlayingSim) {
      // If we are simulating, toggle simulation play state
      ...
      // Manual control play/pause
      const nextPlaying = !matchState.isPlaying;
      
      setMatchState((prev) => {
        let nextStatus = prev.status;
        let nextMin = prev.minute;
        let nextSec = prev.seconds;
        if (nextPlaying && (prev.status === 'UPCOMING' || prev.status === 'HT' || prev.status === 'HALF_TIME' || prev.status === 'FINISHED' || prev.status === 'FT')) {
          if (prev.status === 'UPCOMING' || prev.status === 'FINISHED' || prev.status === 'FT') {
            nextStatus = '1H';
            nextMin = 0;
            nextSec = 0;
          } else if (prev.status === 'HT' || prev.status === 'HALF_TIME') {
            nextStatus = '2H';
            nextMin = 45;
            nextSec = 0;
          }
      ...
      try {
        let nextStatus = matchState.status;
        let nextMin = matchState.minute;
        let nextSec = matchState.seconds;
        if (nextPlaying && (matchState.status === 'UPCOMING' || matchState.status === 'HT' || matchState.status === 'HALF_TIME' || matchState.status === 'FINISHED' || matchState.status === 'FT')) {
          if (matchState.status === 'UPCOMING' || matchState.status === 'FINISHED' || matchState.status === 'FT') {
            nextStatus = '1H';
            nextMin = 0;
            nextSec = 0;
          } else if (matchState.status === 'HT' || matchState.status === 'HALF_TIME') {
            nextStatus = '2H';
            nextMin = 45;
            nextSec = 0;
          }
        ...
        console.error("Error updating match timer on server:", err);
      }
    }
  };
  ...
  const activeLineup = useMemo(() => {
    // Return custom lineups from manual admin upload/sync directly
    return customLineups[selectedLineupTeam];
  }, [customLineups, selectedLineupTeam]);

  const handleSetSelectedApiMatchId = (val: string) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setSelectedApiMatchId(val);
  };
  ...
  const handleSetStreamSource = (val: 'camera' | 'greenscreen' | 'phone') => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setStreamSource(val);
  };

  const handleSetIsTokenSavingMode = (val: boolean) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setIsTokenSavingMode(val);
  };

  const handleSetPhoneCamStreamId = (val: string) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setPhoneCamStreamId(val);
  };

  const handleSetPinnedMatchIds = (val: string[]) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setPinnedMatchIds(val);
  };

  const handleSetIsAutoSyncActive = (val: boolean) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setIsAutoSyncActive(val);
  };

  const handleSetIsAutoRefreshListActive = (val: boolean) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setIsAutoRefreshListActive(val);
  };

  const handleSetShowOverlayInstructions = (val: boolean) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setShowOverlayInstructions(val);
  };

  const handleSetCompetitionImage = (val: string | null) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setCompetitionImageState(val);
  };

  const handleSetStreamerLogo = (val: string | null) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
    setStreamerLogoState(val);
  };

  const handleSetBackgroundImage = (val: string | null) => {
    lastLocalActionTime.current = Date.now();
    localTimestampRef.current = Date.now();
  ...
  const upcomingMatches = useMemo(() => {
    // Return custom upcoming matches from manual admin input directly
    return customUpcomingMatches;
  }, [customUpcomingMatches]);
  ...
  return (
    <MatchContext.Provider
      value={{
        ...
        pinnedMatchIds,
        setPinnedMatchIds: handleSetPinnedMatchIds,
        pinnedLimit,
        setPinnedLimit,
        isTokenSavingMode,
        setIsTokenSavingMode: handleSetIsTokenSavingMode,
        ...
        selectApiMatch,
        selectedApiMatchId,
        setSelectedApiMatchId: handleSetSelectedApiMatchId,
        selectedDate,
        setSelectedDate: handleSetSelectedDate,
        isAutoSyncActive,
        setIsAutoSyncActive: handleSetIsAutoSyncActive,
        isAutoRefreshListActive,
        setIsAutoRefreshListActive: handleSetIsAutoRefreshListActive,
        ...
        setLineupTeam,
        resetSimulation,
        setStreamSource: handleSetStreamSource,
        streamSource,
        phoneCamStreamId,
        triggerCustomAction,
        setTeamProperty,
        showOverlayInstructions,
        setShowOverlayInstructions: handleSetShowOverlayInstructions,
        competitionImage,
        setCompetitionImage: handleSetCompetitionImage,
        streamerLogo,
        setStreamerLogo: handleSetStreamerLogo,
        backgroundImage,
